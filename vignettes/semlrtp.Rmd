...
we are not aware of using it as the default
...
If the default settings are what desired,
...
of the LRT *p*-value. The usual *p*
values in `lavaan` (and many other SEM
programs) are Wald-based *p*-values,
which is an approximation of the
LRT *p*-value when a parameter is fixed
to zero. They are approximationa and
so can be different from the
LRT *p*-value, and they may also depends on
